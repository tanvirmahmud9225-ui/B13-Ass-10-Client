...
export function MyPropertiesPagination({ totalPage, propertyPage }) {
    ...
    const gotToPage = (newPage) => {
        ...
        setPage(newPage)
        router.push(`/dashboard/owner/properties?page=${newPage}`)
    }
        ...
        <Pagination className="justify-center pt-5 pb-10">
            <Pagination.Content>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                    <Link key={p} href={`/dashboard/owner/properties?page=${p}`}>
                        <Pagination.Item >
